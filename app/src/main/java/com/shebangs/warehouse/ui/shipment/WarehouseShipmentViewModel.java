...
import com.shebangs.warehouse.ui.receipt.ReceiptGoodsInformation;
...
public class WarehouseShipmentViewModel extends ViewModel {
    ...
    private int total;
    private int scannedGoodsNum;
    private float totalPrice;
    private List<ReceiptGoodsInformation> goodsInformationList;
    private boolean isHaveInvalidGoods;
    private String[] deliveryRequiredBranches;
    private String currentDeliveryRequiredBranch;
    ...
    private MutableLiveData<OperateResult> queryDeliveryRequiredBranchResult;
    private MutableLiveData<OperateResult> setCurrentDeliveryRequiredBranchResult;
    ...
    public WarehouseShipmentViewModel() {
        ...
        this.queryDeliveryRequiredBranchResult = new MutableLiveData<>();
        this.setCurrentDeliveryRequiredBranchResult = new MutableLiveData<>();
    ...
    public LiveData<OperateResult> getSetCurrentDeliveryRequiredBranchResult() {
        return setCurrentDeliveryRequiredBranchResult;
    }

    public LiveData<OperateResult> getQueryDeliveryRequiredBranchResult() {
        return queryDeliveryRequiredBranchResult;
    }
    ...
    public String getCurrentDeliveryRequiredBranch() {
        return currentDeliveryRequiredBranch;
    }
    ...
    public void queryCodeInformation(String code) {
        ...
        //检查分店是否有需要发的货物
        if (this.goodsInformationList.size()==0){
            queryCodeInformationResult.setValue(new OperateResult(new OperateError(0,
     ...
     * 查询须发货分店
     */
    public void queryDeliveryRequiredBranch() {
        CommandVo vo = new CommandVo();
        vo.typeEnum = CommandTypeEnum.COMMAND_WAREHOUSE_IN_OUT;
        vo.url = WarehouseInterface.EX_WarehouseBranchInterface;
        vo.contentType = HttpHandler.ContentType_APP;
        vo.requestMode = HttpHandler.RequestMode_GET;
        Map<String, String> parameters = new HashMap<>();
        parameters.put("userkey", WarehouseKeeper.getInstance().userKey);
     ...
     * 查询需发货分店的发货清单
     */
    public void queryDeliveryRequiredBranchGoodsList() {
        CommandVo vo = new CommandVo();
        vo.typeEnum = CommandTypeEnum.COMMAND_WAREHOUSE_IN_OUT;
        ...
        public void execResult(CommandResponse result) {
            switch (result.url) {
                ...
                case WarehouseInterface.EX_WarehouseSubmitInterface:
                    ...
                    break;
                case WarehouseInterface.EX_WarehouseBranchInterface:
                    if (!result.success) {
                        queryDeliveryRequiredBranchResult.setValue(new OperateResult(new OperateError(result.code, result.msg, null)));
                    } else {
                        queryDeliveryRequiredBranchResult.setValue(new OperateResult(new OperateInUserView(null)));
                    }
                    break;
     ...
     * @return 须发货分店名数组
     */
    public String[] getDeliveryRequiredBranches() {
        return deliveryRequiredBranches;
    }
     ...
     * @param value
     */
    public void setCurrentDeliveryRequiredBranch(String value) {
        this.currentDeliveryRequiredBranch = value;
        this.setCurrentDeliveryRequiredBranchResult.setValue(new OperateResult(new OperateInUserView(null)));
    }

    public int getScannedGoodsNum() {
        return scannedGoodsNum;
    }

    public int getUnScanGoodsNum() {
        return total - scannedGoodsNum;
    }
}
